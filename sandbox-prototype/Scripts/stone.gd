...
func _process(delta):
	...
	if scene_node.local_player.global_position.distance_to(global_position) > PICKUP_RANGE:
		return
		# This check is indented under the range test above, after its return, so it can never run. Moving it left one level would enable it.
	...
	var added = Inventory.batch_add_item("Stone", stone_texture, stack_count)
