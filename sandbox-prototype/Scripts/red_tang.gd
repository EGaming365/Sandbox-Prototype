...
func _apply_visuals():
	...
	if sprite:
		...
		var tex_path: String = _texture_map.get(item_type, "res://Assets/Fish_Red_Tang_Raw.png")
...
func _process(delta):
	...
	if player.global_position.distance_to(global_position) <= PICKUP_RANGE:
		...
		if _is_inventory_full(name_to_pick):
			return
		_picked_up = true
		var tex_path: String = _texture_map.get(name_to_pick, "res://Assets/Fish_Red_Tang_Raw.png")
