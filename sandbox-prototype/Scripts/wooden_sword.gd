# Floor item for the wooden sword.
...
func _process(delta):
	...
	if scene_node.local_player.global_position.distance_to(global_position) <= PICKUP_RANGE:
		...
		_picked_up = true
		# Use add_item_with_count so the durability is stored correctly in slot["count"].
		# The batch_add_item function wrongly stacked tool durability values.
		Inventory.add_item_with_count("Sword", sword_texture, durability)
		# Ask the inventory to refresh its display.
