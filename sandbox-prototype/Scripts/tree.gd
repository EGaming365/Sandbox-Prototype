...
func do_chop(chopper_id: int = 1, has_axe: bool = false):
	...
	scene_node.host_spawn_floor_item(drop_pos, "Wood")
	if has_axe:
	...
	if multiplayer.has_multiplayer_peer() and chopper_id != multiplayer.get_unique_id():
		scene_node.notify_chop_cooldown.rpc_id(chopper_id, chop_time)
	if hits >= max_hits:
		...
		return
	await get_tree().create_timer(chop_time).timeout  # per-tree still needed to re-enable THIS tree
