...
func _ready():
	...
	pickaxe_texture = ImageTexture.create_from_image(img)
...
func _process(delta):
	...
	if player.global_position.distance_to(global_position) <= PICKUP_RANGE:
		if _is_inventory_full("Stone Pickaxe"):
			return
		_picked_up = true
		Inventory.add_item_with_count("Stone Pickaxe", pickaxe_texture, durability)
