# Floor item for the copper fishing rod.
...
@export var durability: int = 120

# Image used for this tool in the inventory.
var stone_fishing_rod_texture = preload("res://Assets/Copper_Fishing_Rod.png")
...
func _process(delta):
	...
	if player.global_position.distance_to(global_position) <= PICKUP_RANGE:
		...
		if _is_inventory_full("Copper Fishing Rod"):
			return
		# Mark the tool as collected before touching the inventory.
		_picked_up = true
		# Add the tool with its durability stored as the item's count.
		Inventory.add_item_with_count("Copper Fishing Rod", stone_fishing_rod_texture, durability)
