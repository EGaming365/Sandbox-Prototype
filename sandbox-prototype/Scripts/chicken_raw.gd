...
var item_type: String = "Chicken_Raw"
...
# Hunger restored when this item is eaten.
const HUNGER_RESTORE: float = 30.0
...
func _process(delta):
	...
	if _is_inventory_full("Chicken_Raw"):
		return

	_picked_up = true
	# Move as much of the pile as possible into the inventory. The result is how many items actually fitted.
	var added = Inventory.batch_add_item("Chicken_Raw", chicken_texture, stack_count)
